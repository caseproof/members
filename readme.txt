...
* Fixed: Content Permissions role configuration and the custom error message were visible in REST API responses to any user who could read the post (registered meta is readable regardless of its write auth callback). Both meta keys are now removed from REST responses for users who cannot manage content permissions.
* Changed: The REST protected-posts exclusion now computes its hidden-post list once per request per user instead of for every collection query, and no longer walks post revisions when expanding inherited restrictions.
